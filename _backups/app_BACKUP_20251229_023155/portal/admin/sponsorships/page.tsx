...
export default function AdminSponsorships() {
  ...
  async function allocate(sponsorshipId: string, recipientUserId: string, courseId?: string) {
    ...
    try {
      ...
      setMsg("âœ… Allocated sponsorship to learner.");
      await load();
    } catch (e:any) {
      setMsg("âŒ " + (e?.message || "error"));
    }
  ...
  return (
    <div className="grid">
      <div className="card col-12">
        <div className="h1">Admin â€” Sponsorship Review</div>
        ...
        {sponsorships.map((s)=>(
          <div key={s.id} className="card" style={{ background:"var(--surface2)", marginBottom:10  }}>
            <div className="row" style={{ justifyContent:"space-between"  }}>
              <div><b>{s.kind}</b> â€¢ {s.currency?.toUpperCase()} â€¢ {Math.round((s.amount_cents||0)/100)}</div>
      ...
      <div className="card col-6">
        ...
        {applications.map((a)=>(
          <div key={a.id} className="card" style={{ background:"var(--surface2)", marginBottom:10  }}>
            ...
            <div className="muted">Country: {a.country || "Unknown"} â€¢ Lang: {a.preferred_language || "en"}</div>
            <div className="muted">Accessibility: {(a.accessibility_needs||[]).join(", ") || "None"} â€¢ Economic barrier: {String(!!a.economic_barrier)}</div>
