...
export default function AdminLegacy() {
  ...
  async function setStatus(id: string, status: "new"|"contacted"|"closed") {
    ...
    try {
      ...
      setMsg("âœ… Updated intent status.");
      await load();
  ...
  async function recordGift() {
    ...
    try {
      ...
      setMsg("âœ… Legacy gift recorded as verified.");
...
async function verifyGift(id: string) {
  ...
  try {
    ...
    if (r.matches) setMsg(`âœ… Receipt verified. Receipt # ${r.receipt_number}\nHash: ${r.stored_hash}`);
    else setMsg(`âŒ Receipt hash mismatch!\nStored: ${r.stored_hash}\nRecomputed: ${r.recomputed_hash}`);
  } catch (e:any) {
    setMsg("âŒ " + (e?.message || "error"));
  }
}
  ...
  return (
    <div className="grid">
      <div className="card col-12">
        <div className="h1">Admin â€” Legacy Giving</div>
        ...
        {intents.map((i)=>(
          <div key={i.id} className="card" style={{ background:"var(--surface2)", marginBottom:10  }}>
            <div className="row" style={{ justifyContent:"space-between"  }}>
              <div><b>{i.name || "Anonymous"}</b> â€¢ {i.pledge_type}</div>
              <span className="badge">{i.status}</span>
            </div>
            <div className="muted">{i.email || ""} {i.country ? "â€¢ "+i.country : ""}</div>
      ...
      <div className="card col-6">
        ...
        {gifts.map((g)=>(
          <div key={g.id} className="card" style={{ background:"var(--surface2)", marginBottom:10  }}>
            <div className="row" style={{ justifyContent:"space-between"  }}>
              <div><b>{money(g.amount_cents, g.currency)}</b> â€¢ {g.method}</div>
              <span className="badge">{g.verified ? "verified" : "unverified"}</span>
            </div>
            <div className="muted">{g.received_from || ""} {g.reference ? "â€¢ "+g.reference : ""}</div>
            <div className="muted" style={{ marginTop:6  }}>Receipt: <b>{g.receipt_number || "â€”"}</b></div>
