...
export default function Payments() {
  ...
  async function buy(item: "certificate" | "subscription") {
    ...
    if (!token) {
      setMsg("âŒ Please login first (token missing).");
      return;
    }
    try {
  ...
  return (
    <div className="card">
      ...
      <hr />
      <div className="row">
        <button className="btn primary" onClick={() => buy("certificate")}>Buy Certificate (Â£19.99)</button>
        <button className="btn" onClick={() => buy("subscription")}>Pro Monthly (Â£9.99)</button>
      </div>
